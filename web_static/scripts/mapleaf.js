  ...
  const json = $.getJSON('flights.json', function (index) {
    ...
    for (let i = 0; i < Object.keys(index).length; i++) {
      if (!markers_dict_selected[index[i].FlightID]) {
        ...
        markers_dict[index[i].FlightID].marker.addTo(map).bindTooltip("ICAO: " + index[i].ICAO).bindPopup("HOLA");
        ...
        markers_dict[index[i].FlightID].marker.addTo(map).bindTooltip("ICAO: " + index[i].ICAO).bindPopup("HOLA").openPopup();
        markers.push(markers_dict[index[i].FlightID].marker);
